FILNEU: **Filosofia e Neuroscienze**
CdL Filosofia (magistrale), a.a. 2020/21
*prof. Mario Pirchio*

---

[toc]

---

# Introduzione

*Verrano fornite allo studente le conoscenze necessarie per una piena comprensione di come la **filosofia contemporanea** da una parte, e le **neuroscienze** dall'altra, stiano affrontando lo studio di **percezioni**, **attenzione**, **coscienza** e **libero arbitrio**. Il corso prevede un uso intenso di **articoli originali** di neuroscienze. Sono inoltre previste **lezioni sperimentali** in cui gli studenti replicheranno attivamente alcuni degli esperimenti descritti nei suddetti articoli.*

*Lui personalmente non ha una fede incrollabile nella scienza*
*Stiamo facendo un viaggio insieme, noi e il professore.*

<img src="/home/ludo/.config/Typora/typora-user-images/image-20210217162519731.png" alt="image-20210217162519731" style="zoom:50%;" />

Dove, come e quando nascono attenzione, coscienza e libero arbitrio?
Si possono misurare? Interdizione kantiana

Le *entità misurabili* sono: ioni, canali ionici, potenziale di membrana ecc..
Le *entità non misurabili* sono: mente, pensiero, coscienza, volontà, morale, inconscio, sensazioni

Si possono misurare solamente i loro *correlati neurali*: ma è una semplice correlazione o un rapporto causa-effetto?


# Richiami di Neuroscienze

Nei nostri neuroni entrano ed escono continuamente ioni. 
Attorno ai neuroni abbiamo il cloruro di sodio, il comune sale da cucina.

![Lobi del Cervello](https://www.my-personaltrainer.it/imgs/2018/05/04/lobi-del-cervello-lobo-frontale-e-area-di-broca-orig.jpeg)

<img src="/home/ludo/.config/Typora/typora-user-images/image-20210217180430656.png" alt="image-20210217180430656" style="zoom:50%;" />

Si cerca costantemente di localizzazione i processi mentali, ma essi sono localizzabili o distribuiti nel cervello?

soma = corpo cellulare
Le spine dendritiche si modificano sempre. L'aumento delle conoscenze passa attraverso modifiche hw.

1millesimo di millimetro

Come nutro un oggetto così lungo? Come faccio a fargli arrivare questo materiale?
C'è il *trasporto assonale*. Microtubuli, binari lungo i quali camminano le vescicole.
Le cose che tu produci arrivano dopo più giorni (40 cm al giorno).
Il falso zafferano rovina i microtubuli nei neuroni.

Il coordinatore delle cellule è il sistema nervoso. É sempre chimica, ma particolare.
Si sa benissimo come lavora un neurone, ma non si sa milioni di neuroni lavorano insieme.

Il sangue per ritornare al cuore impiega circa 1 minuto. É un modo di comunicare che va benissimo. L'ipofisi stimola l'ovaio a produrre i follicoli (è una cosa lenta).

É una forma di comunicazione che l'evoluzione non premia. Le cellule che ricevono insulina hanno sulla superficie una sostanza che riconosce l'insulina. 
Neurone: trasmissione selettiva. Le connessioni sono estremamete specifiche.
Le prestazioni mentali sono correlate alle prestazioni neuronali.

La forma serve a permettere una selettività, deposita su pochissime cellule, quelle su cui fa contatto. 

Velocizzare comunicazione: i neuroni sono in realtà delle pile elettriche: fanno viaggiare uns engale elettrico lungo tutto quel metro di assone. Quando arriva in fondo fa uscire la sostanza chimica.
Il segnale elettrico si propaga a 100 m/s, 360 km/h.
Possono venire misurati sia singoli neuroni che popolazioni di neuroni.

**Anatomia cervello**
Alcune parte fanno tante mitosik altre meno. La parte del tubo iniziale diventa il midollo spinale.

Le ripiegature servono ad aumentare la superficie. Lobo frontale e parietale divisi dalla scissura di Roland.

Localizzare funzione, 

Vista sagittale: sezione, taglio
Sporgenza: inion, sotto cervelletto, sopra lobo occipitale

Sostanza bianca e grigia. Intorno all'assone si forma tipo una sfoglia, formando una guaina mielinica. Nella sclerosi multipla la mielina si danneggia.

- sostanza grigia: tutto ciò che non ha mielina. Il soma e i dendriti non hanno mielina.
- sostanza bianca: siamo pieni di assoni coperti di mielina, gli assoni a volte si a volte no

La sost. grigia si ipotizza contenga la maggior prestazione.
Alcune zone svolgono alcune funzioni e solo quelle. 
La membrana cellulare fa passare male gli ioni. Sono 4 gli ioni che entrano ed escono dai nostri neuroni: Na+, Cl-, K+, Ca2+. Il codice neurale

Ci sono delle interruzioni sulla membrana plasmatica: canali ionici: viaggiano questi 4 tipi di ioni.

Le forze sono 2: 

- concentrazione 
- carica elettrica

il cloruro di sodio bagna tutti i neuroni, residuo di vita di mare? 
Il potassio è più abbondante dentro, gli altri 3 fuori

Il potassio tendenzialmente esce, gli altri 3 tendono ad entrare per la *concentrazione*

L'interno del neurone può non essere neutro. Sodio e Calcio entrano sempre.
Canali passivi: sempre aperti, che può succedere?

canali attivi: si aprono solo se succede qualcosa, se arriva ossitocina, serotonina..

Ci sono anche le pompe: prendono dall'interno il sodio, prendono il potassio dall'esterno. Consuma ATP. Senza ossigeno o glucosio si ferma la pompa.
In un neurone a riposo i canali attivi sono tutti chiusi.

L'interno è negativo. Differenza di potenziale elettrico, potenziale di membrana = $V_m = V_i - V_e$ . A riposo il neurone sta a circa $-65 mV$. 

Un neurone iperpolarizzato (con Cl-, va a -75mV) viene *addormentato*, mentre depolarizzato è *attivo*. Ci vuole molto, lo ione non percorre un metro!
Le correnti entrano quasi istantaneamente. Le variazioni che arivano nei neuroni, non sono instantanee, sono lente: 10,20,50 ms. A causa della lentezza di risposta del cambio di potenziae di membrana.. accumulo di ritardi => il presente non esiste.

Potenziale d'azione: propagazione fino da 1m/s a 120 m/s. 
Se si arriva a -45 mV si aprono i canali, quelli Sodio si aprono prima. Supera lo 0. Il neurone può cambiare polarità fino a +30 mV. Poi si apre il potassio, che è positivo. Dura 1 ms il ta-ta, è una scheggia.

Il potenziale arriva lì e si ferma, fa uscire il neurotrasmettitore. Il segnale elettrico. Il trucco del potenziale d'azione è solo chimico. Comunicazione ultrarapida se aumentassimo la velocità dl potenziale d'azione?

Ogni neurone è bersagliato da tantissimi neurotrasmettitori che opssono essere verdi o rossi, eccitatori o inibitori. Sommatore!!!!!!! Ogni neurone è un sommatore!
Bottoni sinaptici, prendi un pezzetto di cervello lo colori e vedi!

Non ci sono neuroni che rilasciano sia acetilcoline che serotonina.
Se arrivo a soglia, produco pot. azione -> rilascio neurotrasmettitori.

Il singolo neurone non parla italiano.
Stimoli tattili/olfattivi. Le entrate sensoriali sono separate, i vari attributi sono separati.
Di fatto c'è una seprazione nelle entrate.

I neuroni tattili si fermano al massimo in un primo bulbo del midollo spinale. Abbiamo tante stazioni in serie. Sequenzialità delle stazioni. Organizzazione somatotipica. 

Tante stazioni rallentano la comunicazione, forse perchè la distanza è troppo lunga? No, il pot. azione è molto veloce.

Concetto: entrate sensoriali sono separate per modalità. In ogni stazione vengono ricalcolati i segnali precedenti.
C'è una somma sempre più grande! Ho le loro proprietà salendo.

Poichè tutto va a convergere verso delle aree che fanno un po' la somma, ci aspetteremmo che ci sia un centro d'arrivo che sintetizza tutto. Non è assolutamente così, non c'è un centro delle ricezioni. Si vedono solo assoni che vanno di qua e di là. 



<img src="/home/ludo/.config/Typora/typora-user-images/image-20210304112028239.png" alt="image-20210304112028239" style="zoom:50%;" />

Nel cervello abbiamo un intrecciamento delle *emiretine nasali* (metà della retina, verso il naso).

# Metodi di studio in Neuroscienze

## Tecniche anatomiche

**Colorazioni di Golgi, di Nissl, di Weigert**

**Imaging con tensore di diffusione (DTI)**

## Tecniche di registrazione dell'attività elettrica

es. 2005 Quiroga

### Registrazioni intracellulari

Microelettrodo che entra dentro al neurone, quando si entra si entra e basta. Al posto di una micropipetta (troppo grande) si usa un microelettrodo (tipicamente di tungsteno, senza liquido, con un filo per chiudere il circuito). 
I potenziali di azione vicino ad un microelettrodo provocano dei rumori forti.

L'animale si immobilizza, con il *curaro* l'acetilcolina viene bloccata e i motoneuroni non rispondono. 
*Blackemore, cat visual cortex*, si *spalettano* varie stimoli ottimi per stimolare i neuroni giusti che si vogliono studiare.

<img src="/home/ludo/.config/Typora/typora-user-images/image-20210310174651689.png" alt="image-20210310174651689" style="zoom:25%;" />Come si isola l'attività dei vari neuorni? Utilizzando una soglia di frequenza. 
<img src="/home/ludo/.config/Typora/typora-user-images/image-20210310180140763.png" alt="image-20210310180140763" style="zoom:33%;" />

Campo recettivo del neurone visivo:
<img src="/home/ludo/.config/Typora/typora-user-images/image-20210310175234572.png" alt="image-20210310175234572" style="zoom:33%;" />
Nei + il gatto produce un potenziale d'azione per lo stimolo visivo auqnado compare; nei - produce pot. azione quando scompare lo stimolo.
Fa una somma fra + e -, si devono tenere conto dei + e - per stimoli perpendicolari.
C'è una computazione pre-conscia, esclusivamente chimica, somma dell'eccitazione e inibizione dei potenziali d'azione. 
Macchine chimiche, la mente si correla con macchinette chimiche.



### Registrazioni extracellulari



### EEG

<img src="/home/ludo/.config/Typora/typora-user-images/image-20210324171355522.png" alt="image-20210324171355522" style="zoom:50%;" />

Inion: osso occipitale, se ci tocchiamo dietro; 
Parte dall'inion...

elettrodi che numeriamo. Tutti i neuroni contribuiscono al segnale dell'EEG. Tutti contribuiscono alla stessa maniera? No, i pricnipalei sono i neuroni piramidali, corterccia cerbrale. 
Cambiando la posizione cambia molto la carica sreigstrata anche su uno stesso neurone, zona di positività e negatività. Si hanno 2 poli, un dipolo elettrico; i dendriti generano un campo elettrico che può essere registrato all'esterno. Si spera che tanti neuroni insieme si sincronizzino. Correlato neurale migliore per trovare correlate dei fenemoni neuroali, *popolazioni di neuroni* che si sincronizzano.

I dipoli elettrici devono essere tutti allineati con l'elettrodo, orientati. Potrebbero anche annullarsi i campi elettrici. Tanti dipoli in tante direzioni annullano i campi elettrici, servono campi elettrici che non si annullano, quindi neuroni paralleli.

Supponendo di mettere un elettrodo, (ci sono circonvoluzioni); un elettrodo

Se il dipolo è perpendicolare viene visto meglio, se è parallelo tende ad annullarsi. 

Limite: nell'EEG rischia di catturare anche l'attività sotto di altre parti e non catturare quelle vicina.

Onde alfa: 8-13 Hz. Il cervello 

Onde Beta: durante il calcolo
Onde Teta: 4-7Hz

Onde delta <4Hz, durante il sonno

### Potenziali evocati

Si misura la differenza di potenziale elettrico fra una coppia di elettrodi:
<img src="/home/ludo/.config/Typora/typora-user-images/image-20210324164138652.png" alt="image-20210324164138652" style="zoom:50%;" />

La risposta ad uno stimolo sarebbe coperta dall'attività di tutti gli altri neuroni; si misura la riposta ad uno stimolo per N volte; si sovrappongono i grafici e si ottengono i potenziali evocati -> si fa la media, ottenendo un grafico più o meno pulito.

Tra 80 e 110 msec troviamo risposta ad uno stimolo acustico.

Già dopo poche stazioni si possono misurare effetti  dell'attenzione.

## Tecniche di Imaging

### Risonanza magnetica funzionale (fMRI)

Rileva piccole variazioni del metabolismo encefalico. 
Che vuol dire che sono più attive? Sensori misurano il rapporto fra ossiemoglobina e deossiemoglobina. I globuli rossi distribuiscono ossigeno a tutte le cellule, grazie all'emoglobina. Quando essa arriva ai polmoni prende l'ossigeno.
Problema: risposta lenta, falsi negativi e falsi positivi
il segnale diminuisce, poi riaumenta intorna ai neuroni. Questo perchè i vasi intorno si dilatano e gli arriva più ossigeno. 
la risposta è ritardata di secondi.
Falsi negativi: salmone morto ma in realtà rilevato positivamente

## Stimolazione magnetica intracranica (TMS)

<img src="/home/ludo/.config/Typora/typora-user-images/image-20210324174131786.png" alt="image-20210324174131786" style="zoom:50%;" />

Eccitazione di neuroni involontaria; dove nasce la volontà se i neuroni possono fare qualcosa in modo involontario?

# Filosofia della mente

<img src="/home/ludo/.config/Typora/typora-user-images/image-20210304111151102.png" alt="image-20210304111151102" style="zoom:50%;" />

## Dualismo, Monismo, Emergentismo

> Sandro Nannini, Introduzione alla filosofia della mente

Cerca di ricostruire schematicamente mente e corpo, un quadro sinottico.
<u>Suonano alla porta, io sento e decido di aprire</u>
Evento esterno produce in me delle percezioni e tutto questo messo insieme mi causa una decisione. 

Ad ogni evento mentale corrisponde 



<img src="/home/ludo/.config/Typora/typora-user-images/image-20210224170938423.png" alt="image-20210224170938423" style="zoom:33%;" />

La vibrazione del timpano non è un processo cerebrale. I processi cerebrali sono quelli che avvengono nell'encefalo.
Che relazione intercorre fra:

<img src="/home/ludo/.config/Typora/typora-user-images/image-20210224171414927.png" alt="image-20210224171414927" style="zoom:33%;" />

e: 

Stimolo -> ..? processi cerebrali ?.. -> Risposta motoria

**Dualismo ontologico, interazionistico**
Aristotele: psiche principio di organizzazione funzionale
Platone: dualismo ontologico, anima immateriale separata dal copro (reincarnazione)
ontologia -> studio dell'essere

1.24.00

Tra Platone e Aristotele portano assieme il dualismo su 2 binari, ontologico res cogitans e res extensa e ** solo attraverso mediazione di oggetti fisici, conosciamo solo le cose rosse, che sono imitazioni del rosso, allo stesso modo per Cartesio. Il contenuto rappresentazioneale degli stati mentalei è conosciuto dirttamente, non abbiamo bisogno di altre misurazione per avere creenze ecc..
Per la res extensa ho bisogno di una mediazione percettiva

Ontologico ed Epistemiologico

Per Cartesio l'anima non è un principio animatore, l'anima interagisce col corpo e lo fa funzionare.
Per Platone le cose stanno insieme.

> L'epistemologia (dal greco ἐπιστήμη, epistème, "conoscenza certa" ossia "scienza", e λόγος, logos, "discorso") è quella branca della filosofia che si occupa delle condizioni sotto le quali si può avere conoscenza scientifica e dei metodi per raggiungere tale conoscenza.
>
> L'ontologia, una delle branche fondamentali della filosofia, è lo studio dell'essere in quanto tale, nonché delle sue categorie fondamentali. Il termine deriva dal greco ὄντος, òntos (genitivo singolare del participio presente del verbo εἶναι, èinai, «essere») e da λόγος, lògos («discorso»), e quindi letteralmente significa «discorso sull'essere».
>
> La gnoseologia (dal greco gnòsis, «conoscenza», + lògos, «discorso»), chiamata anche teoria della conoscenza, è quella branca della filosofia che studia la natura della conoscenza.
> In particolare, così come si è consolidata nell'età moderna ad opera della speculazione filosofica di Kant, la gnoseologia si occupa dell'analisi dei fondamenti, dei limiti e della validità della conoscenza umana, intesa essenzialmente come relazione tra soggetto conoscente e oggetto conosciuto. 
> Occorre precisare che nell'ambito della cultura anglosassone la teoria della conoscenza è chiamata anche e soprattutto epistemology, laddove in Italia con il termine epistemologia si designa essenzialmente quella branca della gnoseologia che si occupa della conoscenza scientifica o, in un senso ancora più specifico, la filosofia della scienza.

I processi mentali vengono scomposti in  2 categorie: correlati neuronali della percezione e della decisione. 

<img src="/home/ludo/.config/Typora/typora-user-images/image-20210224172228062.png" alt="image-20210224172228062" style="zoom:33%;" />

In che modo si può passare alla decisione? I processi cerebrali della decisione vengono causati dalla mente, dall'io.
L'intervento 

Reazione di causazione mentale. Non ci può essere un intervento di qualcosa di immateriale

Obiezione: violazione della chiusura del mondo fisico. 
Stazioni seriali, ci sono delle tappe intermedie, nella tappa percettiva, e ci sono anche nella parte motoria. 

Per gap neurofisiologico intende il mentale che manifesta il *libero arbitrio*.
Dal punto di vista fisioligico il dualismo con il gap neurofisioligico non ha modo di esistere.

2 soluzioni
**Parallelismo**
Mondo mentale e mondo fisico sono 2 paralleli, allora come potrebbero essere sincronizzati così bene?
Sincronismo è solo un'impressione, la percezione del tempo è abbastanza fallace. La percezione e la consapevolezza della percezione non possono essere sincronizzati. 

*Emergentismo, epifonemenismo*
Dal fisico emerge l'attività del cervello. Ma allora la mente non spiega il mio comportamento e non posso osservarne gli effetti, per cui non posso dimostrarne l'esistenza.

**Comportamentismo**
Per la psicologia scientifica esistono solo gli organismi, le situazioni in cui essi si trovano ed il loro comportamento direttamente osservabile.
Trova le correlazioni fra situazioni e comportamenti osservabili. Finchè si parla di esperimenti semplici la cosa può anche funzionare, ma poi si ferma.
É una teoria della mente, che si occupa di spiegare la parte superiore dello schema precedente.
Lasciando stare la parte fisica (res extensa), gli eventi mentali si studiano in questo modo: correlazioni fra input e output senza indagare attività mentali. Poichè questi non sono misurabili scientificamente.
Non si sovrappone all'indagine dei processi mentali. Non posso indagarli dentro. Possiamo solo studiare correlati tra stimolo e risposta.

Soluzione 3: **Teoria dell'identità**
Vogliamo continuare a credere nell'esistenza di processi mentali dell'io
Processi mentali si identificano nei processi fisici. 
<img src="/home/ludo/.config/Typora/typora-user-images/image-20210224174225353.png" alt="image-20210224174225353" style="zoom:33%;" />

Solo ciò che è fisico esiste realmente. Possiamo confrontarlo con quanto detto prima.

<img src="/home/ludo/.config/Typora/typora-user-images/image-20210224174317585.png" alt="image-20210224174317585" style="zoom: 33%;" />

Una riduzione completa di tutti gli atti è quantomai improbabile. Il libero arbitrio si ripresenta.

**Materialismo eliminativo**
<img src="/home/ludo/.config/Typora/typora-user-images/image-20210224174453122.png" alt="image-20210224174453122" style="zoom:33%;" />

Nessun correlato neuronale è correlato fra percezione 
Il linguaggio psicologico è troppo grossolano, incapacità di sintesi. Rimarrà fino a quando psicologia e neurofisiologia finiranno per 
Nega l'adeguatezza dei concetti. 
L'obiezione ricorrente: è fortemente contrario a ciò che la maggior parte della gente crede riguardo ai proprio atti mentali.
Come da vitalismo a meccanicismo.

Con un linguaggio unificato potremmo trovare 

Sol. 5: **Funzionalismo, cognitivismo**
Il dualismo interazionistico e il materialismo sono i due poli estremi.
Lo schema base dei funzionalisti introduce 
Gli atti mentali vengono interpretati come stati funzionali, per lo più ignoto, dal cervello. *funzionale* vs *anatomico*. 
Un certo atto percettivo può essere realizzato con vari processi cerebrali. Alla base del funzionalismo c'è l'analogia *mente-computer*.
Ipotesi connessionistica, delle reti neurali.

L'idea del funzionalismo: se uno indaga i processi mentali vede che sono collegati ad altri. Si chiede che correlazioni ci sono fra questi stati mentali. Il ruolo è individuato dalle interazioni fra i procesi mentali. Se capisco i ruoli
David Marr, individua 3 livelli: computazionale, algoritmico, implementazione. Quello che fa differenza è ciò che la mente implementa. Analogia: hw/sw.
É alla base delle scienze cognitive, messa in discussione dal connessionismo

Non necessariamente dobbiamo adottare l'idea di processi computazionali per uno stato funzionale. Comunque si presta ad una realizzazione multipla. 
Funzionalismo e teoria computazionalista vanno d'accordo.

-> 

Le 3 soluzioni fondamentali: materialismo, funzionalimo, dualismo
secondarie: idealismo, emergentismo. 
Cercate di guardare al nocciolo delle cose: a volte idee risultano apparentemente nuove ma poi sono solo riformulazioni.. tenere gli occhi aperti.

**Emergentismo**
Una singola molecola d'acqua in sè non è nè solida nè liquida. Il legame a idrogeno ci dà la spiegazione fisica del perchè emerge una nuova proprietà.
Tra i neuroni non abbiamo nessuna spiegazione di come ciò avvenga. Sopravviene l'attività del cervello ma non è riducibile.

**Sopravvenienza**
Si incontra in filosofia della mente, sopratutto in metafisica. Serve per una caratterizzazione
Prop A e B, si dice che A sopravviene su B se è impossibile che 2 oggetti differiscano su A e B
le proprietà dell'acqua tattile e le prop. fisiche sopravvengono.
<u>Non è possibile che una differenza dell'attività mentale non sia accompagnata da differenze fisiche</u>. 3 concetti. Salvaguardiamo linguaggi del mentale e del fisico.

1) *necessità*, ci da una condizione di impossibilità. La cosa più evidente è che si sta parlando di proprietà, di predicazioni.

2)

3)

La descrizione del mentale ha una sua dignità, obiettivo: mantenere legittimità della descritzione dei processi mentali. Il livello ontologico è del fisico (res extensa).
Alcune critiche: ancora alcune cose non le consociamo, per trovare la riduzione bisogna tenere conto.

Teoria della mente estesa. Gli stati mentali sono tali perchè hanno contenuto (Brentano). 
Nella riduzione l'obiettivo è non perdere nulla. 

# Percezioni, attenzione, coscienza e libero arbitrio

Quanto possiamo fidarci delle nostre rappresentazioni mentali del mondo esterno, delle nostre percezioni?
I colori sono radiazioni elettromagnetiche. L'udito ha un range di 20-20.000 Hz. Le nostre percezioni sono né affidabili né complete né attendibili.
Per l'*incompletezza*: riguarda il cablaggio delle nostre reti neurali, che può causare illusioni ottiche, uditivie ecc, allucinazioni o vere e proprie patologie come la schizofrenia.

## Primi tentativi

### William James

> Ognuno sa cos'è l'attenzione. É il prendere possesso della mente, in una forma chiara e vivida, di un uno fra quelli che sembrano essere simultaneamente possibili oggetti del pensiero, o treni.
> La focalizzazione, la concentrazione della coscienza sono l'essenza dell'attenzione.
> Ciò implica un ritiro di qualcosa al fine di gestirne efficacemente altre ed è una condizione che trova il suo opposto nella distrazione.

3 principi base dei suoi ragionamenti sulla mente: 

- *volontarietà*: «É il prendere possesso della mente»
- *selettività*: «di un uno fra quelli che sembrano essere simultaneamente possibili oggetti del pensiero»
- *capacità limitata*: «Ciò implica un ritiro di qualcosa al fine di gestirne efficacemente altre»

Il problema è che non definisce i termini, non definisce mente, pensiero o coscienza.

### Pazienti con danni cerebrali

- **ADHD** (Attention Deficit Hyperactivity Disorder)

  - É uno dei disturbi dell'attenzione, forse è dovuto ad alterazioni dei lobi frontali; circa il 5/10% dei bambini in età scolare ne è affetto ed è riconosciuto come un problema verificato tanto che sono affiancati da docenti di sostegno

- **Negligenza spaziale unilaterale**

  - É dovuto a lesioni unilaterali della corteccia frontale, parietale e temporale posteriore, ed è più grave se nell'emisfero destro.

  - Il paziente trascura (*neglette*) l'informazione che raggiunge l'emisero colpito (proveniente perciò dall'*emicampo* (metà campo) visivo controlaterale).

    <img src="/home/ludo/.config/Typora/typora-user-images/image-20210304112028239.png" alt="image-20210304112028239" style="zoom:50%;" />

    Nel cervello abbiamo un intrecciamento delle *emiretine nasali* (metà della retina, verso il naso).

  - <img src="/home/ludo/.config/Typora/typora-user-images/image-20210304112304872.png" alt="image-20210304112304872" style="zoom:33%;" />

    Test per la diagnosi della *negligenza spaziale unilaterale*

  - Anche durante l'immaginazione, l'informazione rievocata raggiunge l'emisfero colpito, poichè il paziente non è cieco in quell'emicampo visivo. (es. duomo di Milano da 2 diverse angolazioni, l'emicampo non immaginato è lo stesso ma il paziente conosce tutta la piazza e riesce ad immaginarla spostandosi di angolazione)

  - Fenomeno dell'*estinzione*, ovvero la presenza simultanea di 2 stimoli, uno per emicampo

- **Sindrome di Bálint** (Simultagnosia)

  - É un'incapacità di percepire simultaneamente oggetti o eventi; vengono riconosciuti gli oggetti ma uno alla volta. Percepiscono il mondo ma non nella sua totalitá, bensì come una serie di oggetti, senza vedere la scena nella sua totalità

### **Attenzione selettiva. Orientamento**

É la capacità di selezionare fra vari stimoli esterni o interni presenti contemporaneamente, uno o più di essi, in modo da elaborarlo in modo più efficace

É una conseguenza della limitazione delle risorse umane disponibili: non tutti gli stimoli sensoriali possono essere elaborati contemporaneamente. 

*Oss:* tutti gli stimoli sensoriali vengono elaborati dalle prime stazioni sensoriali (visive, uditive, ecc..) ma *solo quelli a cui si presta attenzione diventano poi percezioni coscienti*.

L'attenzione si divide in *riflessa o volontaria*, in *esplicita/non esplicita*.

**Attenzione volontaria, endogena**
Prestare attenzione a qualcosa intenzionalmente, è un processo top-down

**Attenzione riflessa, esogena**
Processo bottom-up, es. esplosione, lampo di luce

**Attenzione esplicitia (manifesta, overt attention)**
Spostare lo sguardo verso uno stimolo

**Attenzione non esplicita(non manifesta, covert attention)**
Mantenere testa ferma e occhi fissi su un punto e prestare attenzione a un altro punto segretamente.
<img src="/home/ludo/.config/Typora/typora-user-images/image-20210304114252570.png" alt="image-20210304114252570" style="zoom: 50%;" />

L'attenzione visiva è come un lampione, mentre quella uditiva è un filtro.

*Helmholtz*, quando si fissa qualcosa si guarda anche una parte al di fuori, presti attenzione ad una parte periferica del campo visivo in modo nascosto. Il soggetto notava una after image, il movimento degli occhi non era importante. 
Anche nell'uditivo si nota qualcosa di analogo; durante un cocktail party lui apparentemente ascoltava l'interlocutore ma in modo nascosto dirigeva attenzione verso qualcos'altro. Esperimenti: input diversi a orecchio sinistro e destro. L'attenzione concentrata su un input di un solo orecchio non rendeva raggiungibili entrambi gli input. In qualche maniera influenzo la mia attenzione. Dove agisce?

Ipotesi selezione precoce e tardiva.

Le caratteristiche fisiche di tutti gli stimoli solo quelli rilevanti

*?Il processo attentivo elimina davvero gli stimoli irrilevanti?*
Ci sono tante stazioni, non sono in serie, i segnali vanno avanti e indietro. Ogni stazione successiva integra qualcosa. 
Nella prima stazione entra tutto, più si va in là più si ha un taglio. Noi influenziamo la quantità di stimoli che passano.
Sull'uditivo siamo più bravi, centri superiori influenzano la primissima stazione!

**Posner**
1978,1980. Esperimenti banali, 2mm, 5mm a 7 cm dal punto di fissazione una luce.
Paradigma di Posner, paradigma del cueing spaziale

Il 70/80% delle volte la freccia indica la direzione in cui comparirà un quadratino, ma a volte inganna -> distrugge l'aspettativa. Ha misurato come l'attenzione velocizza le risposte. 

Il nostro gnu che sta nella prateria sentendo il leone e prestando attenzione alla direzione che si aspetta si salva, come è più probabile.
Metafora sportiva: finte, induci l'avversario a pensare e concentrarsi ad una determinata scelta.

Cosa stiamo misurando? una capacità del cervello o una capacità mentale?

**Quiroga**

Soglia di riconoscimento conscio. Soglia di riconoscimento ad uno stimolo di 1000 Hz.
Per variare la soglia si aumenta l'intensità.

Dopo l'immagine si fa vedere una maschera, backward masking, per rendere meno sbiadita l'immagine nel tempo poiché rimane più tempo sulla retina. 
Il valore di *soglia* è quello per cui dici "sento/non sento", frequenza del 50%.

Più si aumenta la durata più la frequenza è elevata. A 66ms si ha una maggiore apprendimento.
Si è provato a passare ai singoli neuroni.

Colore rosso: soggetto non vede. Risposte dei singoli potenziali di azione. 
Oss: il potenziale di azione dura molto di più rispetto alla durata dello stimolo.

Anche il neurone si correla con il cosciente l'ho visto, non l'ho visto.

Il risultato principale è che la scarica di questi neuroni . Lo stesso input a seconda delle percezioni risponde diversamente. Se il soggetto presta o no attenzione cambia qualcosa? Secondo loro no, secondo Postner sicuramente sì. 
Il nostro modo di riconoscimento tutto o nulla è soggettivo, possono essere stati fatti errori; il soggetto potrebbe aver detto di riconoscere una persona ma pensava ad un'altra persona. Il neurono da cui stiamo registrando risponde a ciò che lui ha visto o ciò che lui sta immaginando?

Se avessi usato una risonanza avrei potuto confondere, l'area è sempre attiva e la misurazione è più lenta.
con fMRI ci metto secondi, l'ossigeno che deve andare su => risposte dopo 5-6 secondi mentre con i singoli neuroni analizziamo i potenziali di azioni immediati.

Si va "a caccia" di neuroni; stiamo cercando dei neuroni che rispondono allo stimolo della comparsa di un viso. Se risponde a troppe cose non va bene; 

*segnalare la percezione*: potrebbe anche voler dire che sia necessario, forse una casualità. Non sappiamo cosa passa per la mente del soggetto; posso fare previsioni. 
modulata: i numeri di potenz azioni siano più o meno. La percezione *causa*..

nel mondo delle neuroscienze c'è un mescolamento.

**Hipp**
Ogni metodo ha dei difetti. 
Sincronizzazioni oscillatorie su cortecce neuronali a larga scala predice la perceczione.

Usano EEG, 
Cubo di Necker: 2 percezioni diverse, a volte va in su, a volte va in giù. La domanda è: posso misurare qualcosa di oggettivo, e misurando posso dire che il soggetto sta vedendo il cubo verso su o giù?

barre: non è prevedibile se il soggetto percepisce un *bounce*(scontro) o un *passaggio* delle sbarre. Sperano di predire quello che lui avrà dopo attraverso le misurazioni. 

Scopo anche medico/riabilitativo, mentre qui è di ricerca. Esperimento sottile. Come se il cervello si stesse preparando a ricevere bounce o pass. 

Ci sono sottoinsiemi di neuroni che si sincronizzano tra di loro. L'EEG ci fa vedere attività locali. Finora non si sono avute molte info sulla percezione; forse perchè la percezione ha bisogno di sincronizzazione contemparanea.

misurano la coerenza.  I due ** probabilità con Anova avente p molto bassa. Si trova una coerenza che ci permette di dire: se è alta si ha bounce; se è bassa si ha 
Guardando la coerenza *prima* (e non solo dopo) predico cosa

Fluttuazioni a lunga distanza che precedono ambiguità sensoriale.  

Non è stato contraddetto. Sono in grado di predire cosa proverai. 

Coerenza: neuroni si stanno sincronizzando? rumore di fondo altroché se è correlato; materialismo che causa il mentale?

Non sono tante le illusioni percettive con cui si ha la possibilità di definire un istante 0, con  cui misurare il prima e il dopo.

-> controllare il concetto di coerenza.
Tramite EEG e usando questa tecnica si mette in luce la loro bravura e una nuova modalità di usare uno stumento. 



Attenzione 24/03

**Hillyard**

![image-20210324164748625](/home/ludo/.config/Typora/typora-user-images/image-20210324164748625.png)

Gli ascoltatori umani sono capaci di confinare la loro attenzione ad un singolo messaggio acustico.
Meccanismi che sopprimono informazioni provenienti da altri stimoli sensoriali

Stati preparatori non selettivi. Usano una sequenza random???

Correlati neurali

Esperimento: 

- sequenza di suoni pips a 800Hz, 50db -> tra 250 e 1250ms in un orecchio e nell'altro 1500Hz
  - si devono contare i toni più alti

hanno dimostrato che l'attenzione che viene studiata 

VEP: visual evoked potentials

Mastoide: osso sotto l'orecchio.
Se avesse chiuso gli occhi le onde alfa avrebbero coperto i segnali (sarebbero state necessarie moltissime prove).  Occhi
Con l'uditivo si devono tenere gli occhi aperti. Quando siamo attivi abbiamo un'attività Beta, desincronizzata, tipica dell'attività mentale.

<img src="/home/ludo/.config/Typora/typora-user-images/image-20210324171006728.png" alt="image-20210324171006728" style="zoom:50%;" />

<img src="/home/ludo/.config/Typora/typora-user-images/image-20210324171243325.png" alt="image-20210324171243325" style="zoom:50%;" />





*?Anche a livello di singoli neuroni posso vedere questo?*
**Moran & Desimone**
Scimmie. 
Siamo consapevoli solo di una piccola quantità di informazioni, qualcosa deve essere tagliato.

<img src="/home/ludo/.config/Typora/typora-user-images/image-20210324171647591.png" alt="image-20210324171647591" style="zoom:50%;" />

La quantià di informazione va diminuita andando centralmente (verso TEO, TE); loro paradossalmente 
Aumento dell'attività non implica aumento dell'informazione. 
I campi recetivi si trasformano.
Un neurone quando viene isolato? 

- dobbiamo essere sicuri che il microelettrodo sia vicino e stia registranod
- campo recettivo sia giusto
  - attività elettrica attribuibile solo a tale neurone

vogliono trovare il campo recettivo dei neuroni: 

<img src="/home/ludo/.config/Typora/typora-user-images/image-20210324172124424.png" alt="image-20210324172124424" style="zoom: 50%;" />

Se la sbarretta è orizzontale e rosse il neurone produce pot. azione; se la mettono verde non produce pot.azione.
Dopo 8 o 16 prove gli si "dice" concentrati sull'altro

Come dirlo ad una scimmia? Math-to-sample task: 

- si presenta al soggetto uno stimolo campione, poi un altro (test) e si verifica se c'è stata 
- <img src="/home/ludo/.config/Typora/typora-user-images/image-20210324172601212.png" alt="image-20210324172601212" style="zoom:50%;" />

Conclusione: cambia solo l'attenzione; quando l'attenzione è diretta/????
Attenzione ridotta

Altra prova: fuori dal campo recettivo; è necessario che lo stimolo sia nel campo recettivo. 

**Bestmann, Blakemore**

Nuova tecnica: stimolazione transcranica (TMS).
Percezione conscia dipende non soltanto dall'input sensoriale ma anche dall'attenzione -> top-down influences. Percetti illusori, fosfeni. Andando direttamente in corteccia: percetti artificiali!
Si mettono in moto meccanismi per indure percezioni.
Fosfene cosciente (uno se ne rende conto) quando la posizione era attesa

Stimolazione transcranica induce fosfeni; si pensa dalle zoni percettive precoci. Sul campo visivo controlaterale da quello stimolato; riflette eccitazione retinotropica; se accendo l'area 17 a sinistra vedo luci a destra...

Percezione dei fosfeni: sonda per misurare capacità sensoriale.
Percetti che scavalcano vie visive.. 
Esperimento: Posner + TMS. L'intensità necessaria è la soglia (diminuire suono fino a che non lo si sente più). Il minimo rumore permette la salvezza a livello evolutivo.

Se troviamo che la soglia è influenzata dall'attenzione => ha effetto diretto sulla corteccia visiva

L'attenzione spaziale suggerita amplifica anche i fosfeni..

<img src="/home/ludo/.config/Typora/typora-user-images/image-20210324175611736.png" alt="image-20210324175611736" style="zoom:50%;" />

<img src="/home/ludo/.config/Typora/typora-user-images/image-20210324175825113.png" alt="image-20210324175825113" style="zoom:50%;" />

<img src="/home/ludo/.config/Typora/typora-user-images/image-20210324180050018.png" alt="image-20210324180050018" style="zoom:50%;" />

Stimolo nell'area occipitale. Potrei avere una consapevolezza di me illusoria?

**Dennett**
2016, What is the bandwidth of perceptual experience?

<img src="/home/ludo/.config/Typora/typora-user-images/image-20210324180452705.png" alt="image-20210324180452705" style="zoom:67%;" />

Una grande quantità di infomarzione ma ciò che possiamo percepire e immagazzinare è limitata.
Vediamo ancora meno di quello che pensiamo. Prese insieme 
Percepiamo un numero di stimoli ridotti rispetto a quelli a cui siamo esposti.
It's easy to miss something if you're not looking.

## Nella Filosofia contemporanea

<img src="/home/ludo/.config/Typora/typora-user-images/image-20210331091034903.png" alt="image-20210331091034903" style="zoom:50%;" />

### Dennett

#### The role of consciousness in self-control

*Seminario del 26 febbraio 2021*
<img src="/home/ludo/.config/Typora/typora-user-images/image-20210226182635408.png" alt="image-20210226182635408" style="zoom:25%;" />



*David Chalmers*: account of the hard problem
Il percorso troncato per l'hard-problem. 

Come si arriva all'hard problem?

- ci si concentra ai percorsi in entrata
- finchè  non hai rozzamente   spiegato come l'informazione o il contenuto arriva alla coscienza (NCC: neural correlates of consciousness)
- hai vinto, lasci il resto ai filosofi
- questo è solo la metà della strada che si deve spiegare per la coscienza!

E poi cosa succede?
Gli effetti di diventare coscienti di qualcosa sono *innumerevoli, importanti* e i meccanismi fisici rimarranno sempre inaccessibili per te. Servirà sempre una scienza in 3° persona per spiegarli.\

**Debner, Jacoby 1994, Word stem completion**
*A process-dissociation procedure (L. L. Jacoby, 1991) was used for separately
estimating the contribution of conscious and unconscious perception to performance of a
stem-completion task*.

nel 1990: 
> In their experiment, words flashed for a brief duration were "sandwiched" between words presented for a longer duration (i.e., one before and one after the briefly flashed word), followed by presentation of a word stem that subjects were to complete. 
> On some trials, the flashed word could be used to complete the stem (e.g., elastic;
> ela), whereas on other trials it could not (e.g., lattice; ela). 
>
> Results showed that flashing a word increased the likelihood of its being given as a completion, even though subjects professed to be unaware that the completion had been flashed. 
> Thus, the findings reported by Forster et al. (1990) revealed a dissociation between effects on a direct measure (subjective report) and an indirect measure (completion performance) of perception.

Word stem completion: fri____
*masked prime*: primo mascherato, la probabilità che sceglierai *friday* piuttosto che *friend* è sollevata dalla masked prime.
Se eri spaventato, se leggi *scared* poco prima, *frightened* sarebbe stata molto alta. La chiave di ciò è la latenza di questa maschera.

[Unconscious Perception: Attention, Awareness, and Control
James A. Debner and Larry L. Jacoby, 1994]



É in pratica impossibile che una marionetta controlli una marionetta che controlla una marionetta ecc.. concettualmente si

Ma un umano può controllare se stesso come se fosse una marionetta

Possiamo pensare a qualsiasi cosa. 

*Intelligence is knowing what to do when you don't know what to do* 
-Piaget

Definizione elegante e la chiave o per imparare self-control

degrees of control: 

<img src="/home/ludo/.config/Typora/typora-user-images/image-20210226180526244.png" alt="image-20210226180526244" style="zoom:33%;" />

self-control is an art, not an algorithm
love your strings to have free will. It's a job to protect your strins (phone)

Il più grande pericolo è che qualcuno controlli le tue stringhe

Wagner: exposed for first *tme what ewe re;y on when we take ourselves to take voluntary emotions e.* psychologist putting diseases

## 

### Sellars

### Searle

### Davidson

## Nelle **Neuroscienze** 

# Articoli di Neuroscienze

## **Owen 2006, vegetative state**

Lo stato vegetativo è problematico, designa stati in cui pazienti che si svegliano dal coma non mostrano consapevolezza.
Attraverso stimoli esterni si va a vedere se ci sono evidenze.
Stacchiamo o non stacchiamo la spina? risponde a stimoli esterni? e se no, il cervello risponde? 

Una paziente di 23 anni, con ritmi sonno-veglia preservati. Secondo le norme internazionali è diagnosticata come in *stato vegetativo*.
Allora noi abbiamo usato la risonanza magnetica funzionale durante la presentazione di frasi parlate (es: *c'era latte e zucchero nel suo caffè*)per vedere quali zone del cervello "si accendono". 

Allora un'altra fMRI dando delle istruzioni: "immagina di giocare a tennis". Se non ha solo sentito questa frase, ma l'ha anche capita, fa partire mentalmente il processo di "gioco", oppure "immagina di visitare le stanze della tua casa". In contrasto quando le si chiedeva "immagina di camminare". Queste risposte neuronali sono indistinguibili da attività di persone "sane".

Questo esperimento mostra che la paziente ha ancora la capacità di rispondere attraverso stimoli neuronali.
Se non ci fossero state queste evidenze ovviamente non si poteva usare come evidenza per mancanza di consapevolezza.

Questo esperimento ha portato a un cambio di prospettiva dello *stato vegetativo*.

**Nachev 2007, critica**
L'attivazione dei neuroni, la presenza dell'attività, 

**Greenberg 2007, critica, psicologia**
Lo studio soffre di debolezze
Il paziente potrebbe non aver capito nulla della frase. Gli autori concludono che il paziente ha preso una decisione, consciamente. Non potrebbe essere che l'attività del cervello sia generata automaticamente alla parola *tennis*?
*Immagina di visitare le stanze della tua casa dopo aver gocato a tennis.*

**Owen 2007, risposta**
riprova e va bene lo stesso

Una persona che dorme non è cosciente.

## He, aree visive

![image-20210310172200435](/home/ludo/.config/Typora/typora-user-images/image-20210310172200435.png)
![image-20210331131625899](/home/ludo/.config/Typora/typora-user-images/image-20210331131625899.png)

Articolo 3: Quiroga
Vedi anche Gazzaniga pg. 241-243.

![image-20210331133141791](/home/ludo/.config/Typora/typora-user-images/image-20210331133141791.png)

## Epilessia

Hanno impiantato elettrodi a persone intrattabili per epilessia, e curandole le hanno anche analizzate. Hanno analizzato risposte dei neuroni dell'ippocampo, amigdala e altro.
Stimoli, 1 secondo, random

Lobo temporale mediale, possono rappresentare ifnormazioni di alto livello i namenoira astrata? caratterizaate dall'invazriazanna alle caratteristiche metriche dell'immagien.

I neuroni possono correlare, si pongono il problema della *rappresentazione astratta*. 993 neuroni (343 singole e 650 multi-unit).  
Hanno usato facce come stimole e hanno misurato 3 differenti tempi di reazione (300ms, 600ms, 1s). 

## 31 Marzo

<img src="/home/ludo/.config/Typora/typora-user-images/image-20210331175436283.png" alt="image-20210331175436283" style="zoom: 67%;" />



# Esperimenti

**Donders**

> Mentre la filosofia si occupa in astratto della contemplazione dei fenomeni mentali la fisiologia, avendo a disposizione i risultati della filosofia, deve investigare la relazione fra questi fenomeni e le azioni del cervello.

È una divisione forse un po' troppo marcata, secondo il professore i due ambiti, filosofia e scienze in genere dovrebbero unirsi e ci si dovrebbe trovare nel mezzo per avere un buon punto di vista.

**Donders 1868**
Metodo sottrattivo per misurazioni indirette del lato sinistro del Canyon.

<img src="/home/ludo/.config/Typora/typora-user-images/image-20210304170815966.png" alt="image-20210304170815966" style="zoom:50%;" />

<img src="/home/ludo/.config/Typora/typora-user-images/image-20210317113117287.png" alt="image-20210317113117287" style="zoom:50%;" />

Per fare le sottrazioni in modo da misurare tempo di elaborazione stimolo e risposta è meglio usare la mediana, poiché prendendo il valore che sta in mezzo minimizzo le differenze. 

Cosa misuriamo con C-A? 
Se rispondiamo prendiamo una posizione tra misura di una prestazione mentale o tempo di elaborazione di una popolazione neuronale.

Non è un test automatico, ma un test in cui noi facciamo una scelta consapevole. 

## Posner

Guardare al centro, punto di fissazione.



## Un po' di statistica

Data una popolazione di eventi (molti) si ottiene una distribuzione gaussiana, caratterizzata da una curva a campana:

<img src="https://www.okpedia.it/data/okpedia/curva-gaussiana-normale-standardizzata.gif" alt="Distribuzione normale ( statistica ) - Okpedia" style="zoom: 67%;" />La posizione della campana dipende dalla media. 
La forma (scampanatura) dipende dalla *varianza*, ovvero la *deviazione standard* al quadrato.
*deviazione standard*: $\sqrt{\frac{\sum(x_i - \bar x)^2}{n-1}}$ (su campioni, su popolazione si divide per *n*).
Con 1 deviazione standard sitamo nel 68%, con 2 nel 95%, con 3 nel 99.7% degli eventi.

*Mediana*: misurazione che sta al 50% dei valori, nel mezzo del grafico insomma, se è simmetrico corrisponde alla media

*Moda*: è il valore che si presenta più spesso (valore più presente)

Quando si fa una gaussiana coincidono media, mediana, moda.
In una distribuzione asimmetrica no.

Statistica *parametrica*: richiede molti campioni, distribuzione gaussiana ecc..
Statistica *non parametrica*: richiede pochi o molti campioni, distribuzione anche non gaussiana (purché non troppo asimmetrica).

### Test statistici

Vogliamo *credere* ai dati sperimentali, ma è un grande problema.
Si procede partendo da un *ipotesi nulla*: i dati relativi ai vari soggetti provengono dalla  stessa distribuzione (stessa popolazione), ovvero le medie dei loro tempi di reazione non sono statisticamente differenti. 

Verifichiamola con 

- ANOVA, per stat. parametrica; l'analisi della varianza
- Kruskal-Wallis, per stat. non parametrica

Nei laboratori si affidano ai matematici, se ci sono. O ci si affida a dei test già pronti, es:
https://astatsa.com/OneWay_Anova_with_TukeyHSD/

Nell'ANOVA:
abbiamo un valore *p-value*: vorremmo sia inferiore a 0.05. 
L'ipotesi nulla non è vera solo se p vale meno dello 0.05% e quindi suggerisce che uno o più statistiche sono significativamente differenti.
Perchè si prende p < 0.05? Matematicamente, su 100 prove 5 saranno sbagliate, e ci sarà una sovrapposizione. 

# Bibliografia consigliata

*Testi consigliati per la parte di filosofia contemporanea (vedi il "Programma").*
A1) Chalmers D. 1995. Facing up to the hard problem of consciousness. J. Conscious. Stud. 2:200–219.
A2) Churchland P. M. 1981. Eliminative Materialism and the Propositional Attitudes. Journal of Philosophy. 78: 67-90.
A3) Dennett D.C. 2018. Facing up to the hard question of consciousness. Phil.Trans. R. Soc. B373: 20170342.
A4) Davidson D. 1970. Mental Events, in Actions and Events, Oxford, Clarendon Press, 1980.
A5) Nagel T. 1974. What Is It Like to Be a Bat? The Philosophical Review. 83:435-450.
A6) Noë A. 2009 - Perché non siamo il nostro cervello. Una teoria radicale della coscienza. Cortina Raffaello.
A7) Searle J. R. 2000. Consciousness. Annu. Rev. Neurosci. 23:557-578.
A8) Sellars W. 1956. Empiricism and the Philosophy of Mind. In Feigl and Scriven, eds. Vol. I, pp. 253-329, 1956.
A9) Altri testi che saranno indicati durante il corso.

*Testi consigliati per la parte di neuroscienze (vedi il "Programma").*
B1) Gazzaniga, Ivry, Mangun. Neuroscienze cognitive. Editore: Zanichelli. Seconda edizione italiana.
B2) Kandel, Schwartz, ecc. Principi di neuroscienze. Casa Editrice Ambrosiana - Zanichelli. Quarta edizione. Volume unico.
B3) Watson, Breedlove. Il cervello e la mente. Le basi biologiche del comportamento. Editore: Zanichelli. 
B4) Purves, Cabeza, Huettel, LaBar, Platt, Woldorff. Neuroscienze cognitive. Editore: Zanichelli. Seconda edizione italiana.

Solo alcuni capitoli dei suddetti libri costituiranno materia di esame: indicazioni precise verranno fornite durante il corso.